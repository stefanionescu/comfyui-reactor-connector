...
from .sdk_errors import phase_error
...
class SessionEvents:
    ...
    def __init__(self, transport: Transport) -> None:
        ...
        self.ready = False
        self.phase = "connect"
        self.diagnostic: FailureDetails | None = None
    ...
    def attach(self) -> None:
        ...
        for event, handler in (
            ...
            self.transport.on(event, handler)
    ...
    def on_error(self, error: object) -> None:
        ...
        self.diagnostic = self.diagnostic or FailureDetails.from_error(self.phase, error)
        self._fail(phase_error(error, self.phase))

    def on_message(self, message: object) -> None:
        """A command acknowledgement alone does not prove model acceptance."""
        if isinstance(message, dict):
            envelope = cast(dict[str, object], message)
            kind = envelope.get("type")
            if isinstance(kind, str) and len(self.message_types) < 16:
                self.message_types.add(kind[:128])
            data = envelope.get("data")
            if envelope.get("type") in ("state", "state_update") and isinstance(data, dict):
                self.state = cast(dict[str, object], data)
        ...
        if (
            ...
            and cast(dict[str, object], message).get("type") == "generation_complete"
        ):
    ...
    def on_recording_window(
        self, start: float, end: float, now: float, predicted_wait: float
    ) -> None:
    ...
    async def command(self, name: str, data: dict[str, object]) -> None:
        """Observe both correlated command replies and separately delivered errors."""
        await self.command_reply(name, data)

    async def command_reply(self, name: str, data: dict[str, object]) -> object:
        """Return a reply only after checking separately reported model rejection."""
        self.check()
        reply = await self.call(name, self.transport.send_command(name, data))
    ...
    async def call[T](self, phase: str, operation: Coroutine[object, object, T]) -> T:
        ...
        try:
            ...
            self.diagnostic = self.diagnostic or FailureDetails.from_error(phase, error)
