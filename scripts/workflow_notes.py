...
def setup_steps(example: Example) -> list[str]:
    ...
    if example.video:
        steps.append("Upload a video in **Upload your source video** (at least 33 frames).")
    if example.image:
        subject = "a portrait" if example.model == "ltx2" else "an image"
        steps.append(f"Upload {subject} in **Upload your starting image**.")
    if example.reference:
        steps.append("Upload the subject to insert in **Upload your reference subject**.")
    if example.ending_image:
        steps.append("Upload the ending image in **Choose the final frame**.")
    if example.storyboard:
        steps.append("Edit the opening prompt, **1. Soft transition**, and **2. Hard cut**.")
    elif example.prompt_sequence:
        steps.append(
            "Edit the opening prompt, **1. Let sunlight through**, and **2. Enter a clearing**."
        )
...
def model_notes(example: Example) -> list[str]:
    ...
    if example.interactive:
        notes.append(
            "Let recording finish to save the video. **End session** stops early and discards it."
        )
...
def sections(example: Example) -> tuple[str, str]:
    ...
    if example.video and not example.interactive:
        setup += "\n\n" + notes.pop()
    setup += (
        "\n\nRun uses Reactor credits. Select a Reactor node, then choose **Help** "
