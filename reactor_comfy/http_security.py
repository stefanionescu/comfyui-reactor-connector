...
def require_local_request(request: web.Request, *, mutation: bool, multi_user: bool) -> None:
    ...
    try:
        if any(
            ...
            raise ValueError("Private routes require a direct local connection.")
        ...
        if origin is not None:
            ...
            if (
                ...
                raise ValueError("The browser origin does not match this host.")
        if request.headers.get("Sec-Fetch-Site") in {"cross-site", "same-site"}:
            raise ValueError("A different site cannot access private configuration.")
        if mutation and (multi_user or request.headers.get("X-Reactor-Comfy") != "1"):
            raise ValueError("Credential changes require the supported local owner context.")
    except ValueError:
        raise web.HTTPForbidden(
            text="Reactor configuration requires a local, same-origin ComfyUI connection."
        ) from None
