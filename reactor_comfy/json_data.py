...
def parse_json(text: str, *, max_bytes: int = 1_048_576, max_depth: int = 16) -> Json:
    ...
    try:
        value = cast(object, json.loads(text, object_pairs_hook=_unique_object))
        return validate_json(value, max_depth=max_depth)
    except (ValueError, RecursionError) as error:
        raise ConnectorError(
            ErrorCode.INVALID_INPUT, "Enter valid JSON with unique keys."
        ) from error
...
def validate_json(value: object, *, max_depth: int = 16) -> Json:
    ...
    if isinstance(value, dict):
        ...
        if all(isinstance(key, str) for key in entries):
            return {
                cast(str, key): validate_json(item, max_depth=max_depth - 1)
