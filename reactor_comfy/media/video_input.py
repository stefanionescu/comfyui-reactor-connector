...
from .file_output import FileOutput, owned_io
from .process import EncoderProcess
from .video_components import prepare_components
...
async def _copy_source(source: str | io.BytesIO, destination: Path, maximum: int) -> None:
    if isinstance(source, io.BytesIO):
        ...
        return
    path = Path(source)
    valid = path.is_absolute() and await owned_io(
        lambda: path.is_file() and path.stat().st_size <= maximum
    )
    if not valid:
        raise input_error()
...
@asynccontextmanager
async def prepared_video(
    video: Input.Video, settings: Settings, temporary_root: Path
) -> AsyncGenerator[Path]:
...
async def _prepare_file(
    ...
    worker = EncoderProcess(
        [
            sys.executable,
            "-I",
            str(Path(__file__).with_name("video_file_worker.py")),
            str(source),
            str(destination),
            str(start),
            str(duration),
    ...
    if type(frames) is not int or frames < 33:
        raise input_error()
