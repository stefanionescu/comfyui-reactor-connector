...
def state_directory() -> Path:
    ...
    return (
        Path(os.environ.get("XDG_STATE_HOME", Path.home() / ".local" / "state")) / "reactor-comfy"
    )
...
def private_directory(directory: Path) -> None:
    ...
    if directory.is_symlink():
        raise ConnectorError(
            ErrorCode.CONFIGURATION, "The state directory cannot be a symbolic link."
        )
...
def read_private(path: Path, *, max_bytes: int) -> bytes:
    ...
    if not stat.S_ISREG(before.st_mode):
        raise ConnectorError(
            ErrorCode.CONFIGURATION, "The requested state file is not a regular file."
        )
    ...
    with os.fdopen(os.open(path, flags), "rb") as stream:
        ...
        if (
            ...
            raise ConnectorError(
                ErrorCode.CONFIGURATION, "The private state file changed while opening."
            )
