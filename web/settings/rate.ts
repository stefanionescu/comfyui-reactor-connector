...
import { type Model, requestModels } from '#web/discovery/api.ts';
...
class CreditDialog {
  ...
  private readonly status = element('p', 'Loading the local credit rate…');
  ...
  private render(): void {
    ...
    for (const model of this.models) {
      ...
      if (valid) {
        const total = (rate * this.duration.valueAsNumber).toLocaleString(undefined, {
