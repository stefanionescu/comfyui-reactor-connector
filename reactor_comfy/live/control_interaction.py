...
class ControlInteraction(CameraInteraction):
    ...
    async def connected(self, transport: Transport, track: Track, events: SessionEvents) -> None:
        ...
        self.action_workers = [
            asyncio.create_task(self._actions(events, kind))
            for kind in ("prompt", "audio_prompt", "pointer")
        ]
        ...
        try:
            while True:
                with self.control_lease.lock:
                    stale = self.control_lease.clock() - self.control_lease.last_seen > 0.75
                if name == "pointer" and self.pointer_active and stale:
                    await self._pointer(events, {"x": 0.5, "y": 0.5, "active": False})
                data = self.control_lease.take_action(name) if self.active else None
                if data is not None:
                    if name == "pointer":
                        await self._pointer(events, data)
                    elif name == "audio_prompt":
                        async with asyncio.timeout(5):
                            await events.command("set_audio_prompt", dict(data))
                    else:
                        if self.control_lease.options.model.startswith("reactor/visko-"):
                            data = {**data, "passthrough": self.control_lease.options.passthrough}
                        async with asyncio.timeout(5):
                            await events.command(
                                "set_shot"
                                if self.control_lease.options.model == "reactor/longlive-v2"
                                else "set_prompt",
                                dict(data),
                            )
            ...
            events.on_error(
                ...
                if isinstance(error, ConnectorError)
                    ...
                    ErrorCode.TIMEOUT, "A live command did not finish. The session is ending."
                )
            )

    async def _pointer(self, events: SessionEvents, data: dict[str, Json]) -> None:
        # Track a press before sending it so cleanup also covers a missing reply.
        self.pointer_active = self.pointer_active or data.get("active") is True
        async with asyncio.timeout(5):
            await events.command("set_pointer", dict(data))
        self.pointer_active = data.get("active") is True
