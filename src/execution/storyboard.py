...
class Shot:
    ...
    def validate(self) -> None:
        ...
        if type(self.prompt) is not str or not self.prompt.strip() or len(self.prompt) > MAX_PROMPT_CHARACTERS:
            raise ConnectorError(ErrorCode.INVALID_INPUT, "Enter a shot prompt of 1 to 20,000 characters.")
...
def parse_storyboard(value: str) -> tuple[Shot, ...]:
    ...
    invalid = ConnectorError(
        ErrorCode.INVALID_INPUT, "Use a storyboard of at most 32 shots with distinct, increasing chunk numbers."
    )
...
class LongLiveRequest(VideoInputs):
    ...
    def validate(self, settings: Settings) -> None:
        ...
        if self.image is not None:
            raise ConnectorError(ErrorCode.INVALID_INPUT, "LongLive accepts shot prompts, not images.")
