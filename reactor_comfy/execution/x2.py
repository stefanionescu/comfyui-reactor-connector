...
def _validate_contract(schema: object, transport: Transport) -> None:
    ...
    commands = {
        ...
        if object_value(object_value(paths.get(f"/events/{name}")).get("post")).get("operationId")
        == name
    }
    sources = [
        track
    ...
    if commands != required or len(sources) != 1:
        raise ConnectorError(
            ErrorCode.UNAVAILABLE, "This X2 input contract is unsupported. Check for an update."
        )
