...
class SanaRequest(VideoInputs):
    ...
    publication: VideoPublication = field(
        default_factory=VideoPublication, repr=False, compare=False
    )

    def validate(self, settings: Settings) -> None:
        validate_capture_inputs(self.duration_seconds, self.seed, settings)
        if type(self.prompt) is not str or len(self.prompt) > 20_000:
        ...
        if type(self.anchor_interval) is not int or not 0 <= self.anchor_interval <= 1000:
            raise ConnectorError(
                ErrorCode.INVALID_INPUT, "Choose an anchor interval from 0 to 1,000."
            )
    ...
    async def configure(self, transport: Transport, events: SessionEvents) -> None:
        ...
        if self.webcam is not None and mode != "camera":
            raise ConnectorError(
                ErrorCode.UNAVAILABLE, "This SANA deployment does not accept webcam input."
            )
        if mode == "file":
    ...
    async def _accept_file(self, transport: Transport, events: SessionEvents) -> None:
        ...
        def observe(message: object) -> None:
            ...
            if (
                ...
                and cast(dict[str, object], data).get("has_video") is True
            ):
                accepted.set()
