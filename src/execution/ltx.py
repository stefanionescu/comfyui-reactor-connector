...
class LtxSpeakRequest(VideoInputs):
    ...
    def validate(self, settings: Settings) -> None:
        ...
        if type(self.script) is not str or not self.script.strip() or len(self.script) > MAX_SCRIPT_CHARACTERS:
            raise ConnectorError(ErrorCode.INVALID_INPUT, "Enter a script of 1 to 10,000 characters.")
        if type(self.words_per_minute) is not int or not 1 <= self.words_per_minute <= MAX_WORDS_PER_MINUTE:
            raise ConnectorError(ErrorCode.INVALID_INPUT, "Use a positive whole-number speech pace.")
        if (
            type(self.image) is not bytes
            or not self.image
            or len(self.image) > settings.max_upload_megabytes * 1_048_576
        ):
            raise ConnectorError(ErrorCode.INVALID_INPUT, "Provide one portrait within the upload limit.")
    ...
    async def configure(self, transport: Transport, events: SessionEvents) -> None:
        ...
        if not minimum <= self.words_per_minute <= maximum:
            raise ConnectorError(ErrorCode.INVALID_INPUT, f"Use a speech pace from {minimum} to {maximum}.")
